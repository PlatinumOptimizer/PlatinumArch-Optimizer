...
EOF
        ...
        "AMD")
            # Imposta modalità prestazioni per AMD
        ...
        "INTEL")
            # Imposta modalità prestazioni per Intel
            echo "max" > /sys/class/drm/card0/device/power_dpm_force_performance_level 2>/dev/null || \
            echo "performance" > /sys/class/drm/card0/device/power_dpm_force_performance_level 2>/dev/null
            ;;
    esac
}
...
apply_kernel_optimizations() {
    ...
    if [ "$STORAGE_TYPE" = "SSD" ]; then
        ...
        echo "1" > /sys/block/sda/queue/iosched/low_latency 2>/dev/null
    else
    ...
    cat > /etc/sysctl.d/10-network-optimizations.conf << EOF
...
EOF
    ...
    # Ottimizzazioni specifiche per grandi quantità di RAM
...
apply_disk_optimizations() {
    ...
    if [ "$STORAGE_TYPE" = "SSD" ]; then
        ...
        echo 256 > /sys/block/sda/queue/read_ahead_kb 2>/dev/null

        # Disabilita access time
        sed -i '/\/dev\/sd/ s/defaults/defaults,noatime,nodiratime/' /etc/fstab 2>/dev/null
    fi
...
apply_network_optimizations() {
    log_platinum "Applicazione ottimizzazioni rete..."
...
EOF
    ...
    # Ottimizzazioni per gaming
    cat > /etc/sysctl.d/10-gaming.conf << EOF
...
apply_extreme_gpu_optimizations() {
    ...
    case "$GPU_VENDOR" in
        ...
        "AMD")
            log_info "Applicazione ottimizzazioni ESTREME per AMD GPU..."
            ...
            if [ -f "/sys/class/drm/card0/device/hwmon/hwmon*/power1_cap_max" ]; then
                MAX_POWER=$(cat /sys/class/drm/card0/device/hwmon/hwmon*/power1_cap_max 2>/dev/null)
                if [ ! -z "$MAX_POWER" ]; then
                    echo $MAX_POWER > /sys/class/drm/card0/device/hwmon/hwmon*/power1_cap 2>/dev/null
                fi
            fi
            ;;

        "INTEL")
            log_info "Applicazione ottimizzazioni ESTREME per Intel GPU..."

            # Set performance level to max
            if [ -f "/sys/class/drm/card0/device/power_dpm_force_performance_level" ]; then
                echo "max" > /sys/class/drm/card0/device/power_dpm_force_performance_level 2>/dev/null
            fi
            ;;
...
enable_services() {
    ...
    # Servizi base
    systemctl enable cpupower.service
    systemctl enable fstrim.timer  # Per SSD
    systemctl enable earlyoom.service
    systemctl enable irqbalance.service
...
main() {
    ...
    echo
    log_warning "Alcune ottimizzazioni richiedono un riavvio per essere effettive."
